"""V11 point-focused neural experiments.

V11A: point loss ablations on the existing stroke-event Transformer.
V11B: structured nonterminal point head with direct 9-class point logits plus
depth/receiver-relative-side auxiliary heads and consistency regularization.

This is a research script. It evaluates whether a neural point branch can
become useful enough to blend with the stable V3/R1 point model.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V11 point-focused sequence experiments.")
    ...
    parser.add_argument("--cv-report", default="cv_report_v11.csv")
    parser.add_argument("--prefix-len-report", default="prefix_len_report_v11.csv")
    parser.add_argument("--point-blend-report", default="point_blend_report_v11.csv")
    parser.add_argument("--class-report-point", default="class_report_v11_point.csv")
    parser.add_argument("--feature-report", default="feature_report_v11.json")
    parser.add_argument("--oof-proba", default="oof_proba_v11.pkl")
...
def point_loss(outputs, batch, point_w, point_counts, point_log_prior, args, device) -> torch.Tensor:
    ...
    if args.variant in {"weighted_ce", "structured_weighted"}:
        return F.cross_entropy(logits, target, weight=point_w.to(device))
    if args.variant == "unweighted_ce":
        return F.cross_entropy(logits, target)
    if args.variant in {"balanced_softmax", "structured_balanced"}:
        return balanced_softmax_loss(logits, target, point_counts.to(device))
    if args.variant == "logit_adjust":
        adjusted = logits + args.logit_adjust_tau * point_log_prior.to(device).unsqueeze(0)
        return F.cross_entropy(adjusted, target)
