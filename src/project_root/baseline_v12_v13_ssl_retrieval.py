"""V12 point-aware SSL + V13 short-prefix embedding-kNN point prior.

V12 trains a compact stroke-event Transformer with point-aware self-supervised
objectives on fold-train prefixes only. V13 uses the learned prefix embeddings
to build a fold-safe kNN point prior for `prefix_len <= 2`, then blends that
prior with the stable V3/R1 point probability.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V12 SSL + V13 short-prefix kNN retrieval.")
    ...
    parser.add_argument("--cv-report", default="cv_report_v12_v13.csv")
    parser.add_argument("--search-report", default="v13_knn_search_report.csv")
    parser.add_argument("--prefix-report", default="prefix_len_report_v12_v13.csv")
    parser.add_argument("--feature-report", default="feature_report_v12_v13.json")
    parser.add_argument("--oof-proba", default="oof_proba_v12_v13.pkl")
    parser.add_argument("--submission", default="submission_v13.csv")
...
def supervised_loss(outputs, batch, action_w, point_w, device) -> torch.Tensor:
    ...
    point10_loss = F.cross_entropy(outputs["point10"], batch["point10"])
...
def train_model(train_arrays, valid_arrays, cat_cards, class_sizes, args, seed, action_w, point_w):
    set_seed(seed)
    ...
    pre_ds = V12PretrainDataset(train_arrays, [card - 1 for card in cat_cards], seed + 9000)
...
def select_knn_config(
    ...
    if best is None:
        selected = {
            ...
            "gain_vs_v3": 0.0,
            "reason": "No eligible V13 kNN config met stopping criteria.",
        }
...
def main() -> None:
    ...
    for fold, (tr_idx, va_idx) in enumerate(splitter.split(rally_meta, groups=rally_meta["match"]), start=1):
        ...
        model = train_model(train_arrays, valid_arrays, cat_cards, class_sizes, args, args.seed + fold, action_w, point_w)
    ...
    pd.DataFrame([{**{f"v12_single_{k}": v for k, v in v12_single.items()}, **{f"selected_{k}": v for k, v in selected_metrics.items()}, **selected}]).to_csv(args.cv_report, index=False)
    ...
    if not args.skip_full_train and selected.get("config") != "base_v3":
        ...
        model = train_model(train_arrays, test_arrays, cat_cards, class_sizes, args, args.seed + 999, action_w, point_w)
        ...
        metadata["submission_note"] = "No V13 config selected; submission not regenerated because point branch remains V3."
