"""Generate R20 top-k action candidate reranker submissions.

R20 is an action-only meta-ranker over top-k candidates. This generator trains
the meta-ranker on the OOF candidate table, then applies it to full test
candidate rows. It writes conservative eta variants for public probing.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate R20 candidate-reranker submissions.")
    ...
    parser.add_argument("--feature-report", default="feature_report_r20_submission.json")
...
def main() -> None:
    ...
    components, _, _ = build_components(v3, v5, v7, v10, r7, r19, selected_v10, selected_r8, selected_r19)
    oof_scores = adjusted_log_scores(meta, components["r19"], selected_r19["action_multipliers"], "two")
    oof_candidates = make_candidate_frame(meta, oof_scores, components, args.top_k)
    ...
    v3_point_pred = apply_segmented_multipliers(
        test_prefix,
        v3_point,
    ...
    r19_point_pred = apply_segmented_multipliers(
        test_prefix,
        r19_point,
        v3["tuning"].point_multipliers,
        POINT_CLASSES,
    ...
    for eta in args.etas:
        ...
        eta_tag = f"{eta:.2f}".replace(".", "")
        safe_path = f"submission_r20_eta{eta_tag}_safe_point.csv"
        r19_path = f"submission_r20_eta{eta_tag}_r19_point.csv"
        ...
        outputs.append(
            {
                ...
                "action_diff_vs_r19_base": float((pred != np.asarray(ACTION_CLASSES)[np.argmax(test_scores, axis=1)]).mean()),
                "point_diff_safe_vs_r19": float((v3_point_pred != r19_point_pred).mean()),
            }
        )

    metadata = {
        "source": "R20 top-k action candidate reranker",
        "etas": outputs,
        "top_k": args.top_k,
        "candidate_features": features,
        "style_feature_count": len(style_features),
        "rows": int(len(test_prefix)),
        "recommendation": "Prefer safe_point eta=0.05 as the first public action-risk probe.",
    }
