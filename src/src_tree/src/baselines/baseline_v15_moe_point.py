"""V15 small tactical-mode MoE for nonterminal pointId.

This is a high-risk point experiment. It keeps a shared stroke-event
Transformer encoder and replaces the nonterminal point head with a small
mixture of experts:

    P(point 1..9 | x) = sum_z P(z | x) * P(point 1..9 | x, z)

The MoE is used only for nonterminal point. Terminal/action/server remain
ordinary heads. Evaluation checks whether this point branch can contribute
positive weight when blended with the stable V3 point model.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V15 small point MoE.")
    parser.add_argument("--train", default="train.csv")
    parser.add_argument("--test", default="test_new.csv")
    parser.add_argument("--v3-oof", default="oof_proba_v3.pkl")
    parser.add_argument("--cv-report", default="cv_report_v15.csv")
    parser.add_argument("--prefix-len-report", default="prefix_len_report_v15.csv")
    parser.add_argument("--point-blend-report", default="point_blend_report_v15.csv")
    parser.add_argument("--class-report-point", default="class_report_v15_point.csv")
    parser.add_argument("--feature-report", default="feature_report_v15.json")
    parser.add_argument("--oof-proba", default="oof_proba_v15.pkl")
...
class StrokeMoEDataset(Dataset):
...
class PointMoETransformer(nn.Module):
    def __init__(
        self,
        ...
        self.expert_gate = nn.Linear(d_model * 2, self.num_experts)
        self.point_experts = nn.ModuleList([nn.Linear(d_model * 2, 9) for _ in range(self.num_experts)])
        self.server_head = nn.Linear(d_model * 2, 1)
        self.parity_head = nn.Linear(d_model * 2, 1)
        self.remaining_head = nn.Linear(d_model * 2, 7)

    def encode(self, cat: torch.Tensor, num: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        return self.head(torch.cat([h_last, h_pool], dim=-1))

    def forward(self, cat: torch.Tensor, num: torch.Tensor, lengths: torch.Tensor) -> dict[str, torch.Tensor]:
        h = self.encode(cat, num, lengths)
        gate_logits = self.expert_gate(h)
        gate_prob = F.softmax(gate_logits, dim=-1)
        expert_logits = torch.stack([expert(h) for expert in self.point_experts], dim=1)
        expert_prob = F.softmax(expert_logits, dim=-1)
        point_prob = (gate_prob.unsqueeze(-1) * expert_prob).sum(dim=1).clamp_min(1e-8)
        return {
...
def moe_point_loss(outputs, batch, point_w, args, device) -> torch.Tensor:
    point_mask = batch["point_mask"] > 0.5
    if not point_mask.any():
        return outputs["point_prob"].sum() * 0.0
    target = batch["point_nonterminal"][point_mask]
    prob = outputs["point_prob"][point_mask]
    weights = point_w.to(device)[target]
    nll = -torch.log(prob[torch.arange(len(target), device=device), target].clamp_min(1e-8))
    hard_loss = (nll * weights).mean()
    mean_gate = outputs["gate"][point_mask].mean(dim=0).clamp_min(1e-8)
    uniform = torch.full_like(mean_gate, 1.0 / args.num_experts)
    balance = F.kl_div(mean_gate.log(), uniform, reduction="sum")
    entropy = -(outputs["gate"][point_mask] * outputs["gate"][point_mask].clamp_min(1e-8).log()).sum(dim=1).mean()
    return hard_loss + args.balance_weight * balance - args.entropy_weight * entropy


def compute_loss(outputs, batch, action_w, point_w, args, device) -> torch.Tensor:
    ...
    point_loss = moe_point_loss(outputs, batch, point_w, args, device)
    server_raw = F.binary_cross_entropy_with_logits(outputs["server"], batch["server"], reduction="none")
    server_loss = (server_raw * batch["server_weight"]).mean()
...
def train_epoch(model, loader, opt, action_w, point_w, args, device) -> float:
    model.train()
    losses = []
    for batch in loader:
...
def predict_model(model, arrays: SequenceArrays, batch_size: int, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    loader = DataLoader(StrokeMoEDataset(arrays), batch_size=batch_size, shuffle=False, num_workers=0)
    model.eval()
    action_parts, point_parts, server_parts, gate_parts = [], [], [], []
    with torch.no_grad():
        for batch in loader:
            ...
            point_nonterm = out["point_prob"].cpu().numpy()
            ...
            gate_parts.append(out["gate"].cpu().numpy())
    return np.vstack(action_parts), np.vstack(point_parts), np.concatenate(server_parts), np.vstack(gate_parts)
...
def load_v3_subset(path: str, valid_meta: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray, V3Tuning]:
    ...
    if merged["index_v3"].isna().any():
        raise ValueError("Could not align V15 valid rows to V3 OOF.")
...
def point_blend_report(valid_meta, v3_point, v15_point, v3_tuning, args) -> pd.DataFrame:
    rows = []
    base_pred = apply_segmented_multipliers(valid_meta, v3_point, v3_tuning.point_multipliers, POINT_CLASSES, v3_tuning.bins_mode)
    for weight in [0.0, 0.02, 0.05, 0.1, 0.2, 0.3, 0.4]:
        prob = blend_probs(v3_point, v15_point, weight)
...
def train_one_fold(train_arrays, valid_arrays, cat_cards, args, fold_seed, action_w, point_w):
    set_seed(fold_seed)
    device = torch.device(args.device)
    model = PointMoETransformer(
        cat_cards,
        len(NUM_FIELDS),
        args.max_len,
        args.d_model,
    ...
    for epoch in range(1, args.epochs + 1):
        loss = train_epoch(model, train_loader, opt, action_w, point_w, args, device)
        _, p, _, gates = predict_model(model, valid_arrays, args.batch_size, device)
        ...
        usage = gates.mean(axis=0)
        print(f"  epoch {epoch:02d}: loss={loss:.5f} point={point_score:.6f} usage={np.round(usage, 3).tolist()}")
...
def main() -> None:
    ...
    device = torch.device(args.device)
    print(f"V15 MoE experts={args.num_experts} device={device} prefix_rows={len(prefix_meta):,}")
    ...
    fold_rows = []
    valid_parts, action_parts, point_parts, server_parts, gate_parts = [], [], [], [], []
    ...
    for fold, (tr_idx, va_idx) in enumerate(splitter.split(rally_meta, groups=rally_meta["match"]), start=1):
        ...
        model, best_point = train_one_fold(train_arrays, valid_arrays, cat_cards, args, args.seed + fold, action_w, point_w)
        a, p, s, g = predict_model(model, valid_arrays, args.batch_size, device)
        metrics = evaluate(va_meta, a, p, s)
        usage = g.mean(axis=0)
        metrics.update(
            {
                "fold": fold,
                "train_rows": len(tr_meta),
                "valid_rows": len(va_meta),
                "best_point_epoch_score": best_point,
                **{f"expert_usage_{i}": float(usage[i]) for i in range(args.num_experts)},
            }
        )
    ...
    v15_action = np.vstack(action_parts)
    v15_point = np.vstack(point_parts)
    v15_server = np.concatenate(server_parts)
    v15_gate = np.vstack(gate_parts)
    single = evaluate(valid_meta, v15_action, v15_point, v15_server)
    v3_action, v3_point, v3_server, v3_tuning = load_v3_subset(args.v3_oof, valid_meta)
    blend_df = point_blend_report(valid_meta, v3_point, v15_point, v3_tuning, args)
    blend_df.to_csv(args.point_blend_report, index=False)
    prefix_report(valid_meta, v15_action, v15_point, v15_server).to_csv(args.prefix_len_report, index=False)
    class_report_point(valid_meta, v15_point).to_csv(args.class_report_point, index=False)
    fold_df = pd.DataFrame(fold_rows)
    fold_df.to_csv(args.cv_report.replace(".csv", "_folds.csv"), index=False)
    cv_report = pd.DataFrame(
        [
            {
                "variant": "small_point_moe",
                "folds_run": len(fold_rows),
                **{f"v15_single_{k}": v for k, v in single.items()},
                ...
                **{f"expert_usage_{i}": float(v15_gate.mean(axis=0)[i]) for i in range(args.num_experts)},
            }
        ]
    )
        ...
        pickle.dump(
            {
                ...
                "v15_action": v15_action,
                "v15_point": v15_point,
                "v15_server": v15_server,
                "v15_gate": v15_gate,
                "fold_report": fold_df,
            },
            f,
    ...
    metadata = {
        ...
        "expert_usage": v15_gate.mean(axis=0).tolist(),
    }
